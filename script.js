/* Meridional Raytracer (2D) — TVL Rebuild (CLEAN + MODERN DRAWING)
   Fixes:
   - HTML IDs match (badges, table, modal)
   - Viewer contrast improved
   - Lens stage highlighted (auto bbox)
   - Glass bodies colored (readable on dark)
*/
...
function computeLensBBox(surfaces){
  ...
  for (const s of surfaces){
    ...
    minY = Math.min(minY, -ap);
    maxY = Math.max(maxY, ap);
  }
  if (!Number.isFinite(minX)) return null;
  return { minX, maxX, minY, maxY };
}

/* 👇 KEY: Lens stage highlight (subtle different tint behind lens area) */
function drawLensStage(world, surfaces){
  const bb = computeLensBBox(surfaces);
  if (!bb) return;

  const padX = 30;        // world mm padding
  const padY = Math.max(10, (bb.maxY - bb.minY) * 0.25);

  const x0 = bb.minX - padX;
  const x1 = bb.maxX + padX;
  const y0 = bb.minY - padY;
  const y1 = bb.maxY + padY;

  const a = worldToScreen({x:x0, y:y0}, world);
  const b = worldToScreen({x:x1, y:y1}, world);
  ...
  ctx.save();
  // slightly lighter zone so lens doesn't vanish
  ctx.fillStyle = "rgba(255,255,255,0.035)";
  ctx.strokeStyle = "rgba(77,124,255,0.12)";
  ctx.lineWidth = 1;
  roundRect(ctx, left, top, w, h, 14);
  ctx.fill();
  ctx.stroke();
  ctx.restore();
}
...
function drawElementBody(world, sFront, sBack, apRegion, tint = 0.18) {
  ...
  // Modern lens-glass feel (blue-ish translucent)
  ctx.globalAlpha = 1.0;
  ctx.fillStyle = `rgba(120, 170, 255, ${tint})`;
...
function drawElementsClosed(world, surfaces) {
  for (let i = 0; i < surfaces.length - 1; i++) {
    ...
    // Slightly different tint if high-index (just a vibe)
    const nd = (GLASS_DB[sA.glass]?.nd ?? 1.5);
    const tint = nd > 1.75 ? 0.22 : 0.16;

    drawElementBody(world, sA, sB, apRegion, tint);
  }
}
...
function drawRays(world, rayTraces, sensorX) {
  ctx.save();
  ctx.lineWidth = 1;
  ctx.strokeStyle = "rgba(90, 150, 255, 0.95)";
...
function drawStop(world, surfaces) {
  ...
  ctx.save();
  ctx.lineWidth = 2.5;
  ctx.strokeStyle = "rgba(255, 82, 96, 0.95)";
...
function renderAll() {
  ...
  if (ui.sensorChip) ui.sensorChip.textContent = `sensor ${sensorW.toFixed(2)}×${sensorH.toFixed(2)}mm`;
  ...
  if (ui.covTop) ui.covTop.textContent = covers ? "COV: YES" : "COV: NO";
...
function refreshGlassNote(){
  ...
  if (modal.note) {
    modal.note.textContent = [
      "UI guidance only. Real pricing depends on supplier, diameter, tolerances, coatings, MOQ.",
      line(g1, m1),
      line(g2, m2),
    ].join("\n");
  }
...
function buildCustomTable(){
  ...
  function blankRow(){ return { type:"", R:0, t:0, ap, glass:"AIR" }; }
...
function renderCustomTbody(){
  ...
  customRows.forEach((r, idx)=>{
    ...
    tr.innerHTML = `
      ...
      <td style="width:110px"><input class="cellInput customCell" data-k="R" data-i="${idx}" type="number" step="0.01" value="${r.R}"></td>
      <td style="width:110px"><input class="cellInput customCell" data-k="t" data-i="${idx}" type="number" step="0.01" value="${r.t}"></td>
      <td style="width:110px"><input class="cellInput customCell" data-k="ap" data-i="${idx}" type="number" step="0.01" value="${r.ap}"></td>
